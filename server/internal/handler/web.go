...
func (h *WebHandler) RecordDetail(c *gin.Context) {
	...
	if err != nil {
		c.HTML(http.StatusNotFound, "error.html", gin.H{"error": "记录不存在"})
		return
	}
...
func (h *WebHandler) TemplateEditPage(c *gin.Context) {
	...
	if err != nil {
		c.HTML(http.StatusNotFound, "error.html", gin.H{"error": "模板不存在"})
		return
	}
...
func (h *WebHandler) ConfigPage(c *gin.Context) {
	c.HTML(http.StatusOK, "config.html", gin.H{"title": "系统配置"})
}